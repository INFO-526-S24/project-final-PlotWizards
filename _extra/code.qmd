...
# Define the UI for the application
ui <- fluidPage(
  titlePanel("Bowling Performance Dashboard"),

  # Dropdown for selecting the season
  selectInput(inputId = "season",
              label = "Select Season:",
              choices = NULL, # Choices will be updated in server logic
              selected = NULL),
...
server <- function(input, output, session) {
  # Update dropdown choices based on available seasons in the dataset
  observe({
    # Make sure 'season' column exists in 'odi_data' and the name is spelled correctly
    seasons <- sort(unique(odi_data$season))
    # Use 'seasons' variable for choices, not 'season'
    updateSelectInput(session, "season", choices = seasons, selected = seasons[1])
  })
  
  # Reactive expression for filtering data based on the selected season
  filteredData <- reactive({
    # Again, make sure 'season' is the correct column name in 'odi_data'
    odi_data %>%
      filter(season == input$season) %>%
      group_by(bowler) %>%
      summarise(
        econ_rate = sum(runs_off_bat) / ceiling(max(ball)),
        .groups = "drop"
      ) %>%
      # Ensure 'bowler' is the correct column name for bowlers in 'odi_data'
      filter(bowler %in% unique(bowler)[1:8]) # Adjust the filtering as needed
  })

  # Render the violin plot
  output$bowlerPlot <- renderPlot({
    ggplot(filteredData(), aes(x = bowler, y = econ_rate)) +
      geom_violin() +
      labs(
        x = "Bowler",
        y = "Economy Rate",
        title = "Distribution of Economy Rates",
        subtitle = "for selected Season"
      ) +
      theme_light()
  })
}
...
server <- function(input, output, session) {
  ...
  filteredData <- reactive({
    ...
    print(data) # Debug: print the filtered data to the R console
    return(data)
  })
  ...
  output$bowlerPlot <- renderPlot({
    ...
    if(nrow(data_to_plot) == 0) {
      print("No data available for the selected season.") # Debug: print a message if no data is available
      return(NULL)
    }
    ggplot(data_to_plot, aes(x = bowler, y = econ_rate)) +
      geom_violin() +
      labs(
        x = "Bowler",
        y = "Economy Rate",
